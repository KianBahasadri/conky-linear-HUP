...
def test_collect_status_sorts_by_severity(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GIT_REPO_PATHS", f"{clean}:{dirty}")
    monkeypatch.setenv("GIT_INCLUDE_STASH", "0")
    status = git_status.collect_status(timeout=5, hide_clean=False, max_repos=10)
...
def test_collect_status_hide_clean(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GIT_REPO_PATHS", f"{clean}:{dirty}")
    monkeypatch.setenv("GIT_INCLUDE_STASH", "0")
    status = git_status.collect_status(timeout=5, hide_clean=True, max_repos=10)
...
def test_collect_status_requires_paths(monkeypatch):
    ...
    assert status["ok"] is False
    assert "GIT_REPO_PATHS" in status["error"]
