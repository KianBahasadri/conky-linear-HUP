...
- The quota panel shows separate `CODEX`, `CLAUDE`, `CURSOR`, `GEMINI`, `GROK`, `OPENCODE`, and `CMD` chips, centered as a row across the panel. Codex rows use cyan/navy bars, with a teal third bar when a reserve window is present; Claude rows use coral/gold bars; Cursor rows use bronze bars; Gemini rows use bright spring green, emerald, and pine green bars on Pro accounts and green/pine bars on Free accounts; Grok rows use black bars; OpenCode Go rows use rose-crimson bars; Command Code rows use indigo bars.
...
- Accounts within each provider are ordered alphabetically by account name without separating free accounts from paid tiers.
