...
- The quota panel shows separate `CODEX`, `CLAUDE`, `CURSOR`, `GEMINI`, `GROK`, `OPENCODE`, and `CMD` chips, centered as a row across the panel. Codex rows use cyan/navy bars, with a teal third bar when a reserve window is present; Claude rows use coral/gold bars; Cursor rows use bronze bars; Gemini rows use Google blue/green and yellow/red bars; Grok rows use black bars; OpenCode Go rows use rose-crimson bars; Command Code rows use indigo bars.
