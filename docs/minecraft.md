...
- The panel sits below sessions in the left rail when enabled. It uses the
  [Desktop design system](design-system.md): player and latency metrics beside
  an `Online` badge, then the player list and optional host resources. An
  unreachable server shows only a danger callout naming the failure, since that
  callout already carries the unavailable status.
