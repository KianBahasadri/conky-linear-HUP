...
- `scripts/install_webdav_service.py` — idempotent installer: generates the
  ...
  `uv run --with bcrypt python scripts/install_webdav_service.py`. It writes
  the current repo and auth paths to
  `~/.config/rclone/webdav-service.env`, so rerunning the installer after the
  repo moves updates the service without editing the committed unit. The
  printed phone URL is the Tailscale HTTPS front door; set
  `WEBDAV_PUBLIC_URL` for the installer invocation if that hostname changes.
