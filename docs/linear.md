...
- Each card carries the project and its state on the first line, a left-aligned
  title, then the identifier with the optional label and due date. Titles start
  at 15px and may use two lines; if they would need a third, the renderer
  retries at 14px then 13px instead of wrapping further, and truncates if 13px
  still overflows. Cards form a gapless grid.
...
- Card width is fluid with a 252px minimum and no gaps. A card is 104px tall
  with a two-line title; a grid row is as tall as its tallest card. Default
  desktop pages have four columns and up to three rows. The renderer applies
  the same due-today visibility filter before allocating page slots.
