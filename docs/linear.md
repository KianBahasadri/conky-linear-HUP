...
- Card headers show only the project name, centered in the bold accent row; the issue title is centered in the main card body, and a footer row along the bottom edge carries the muted issue identifier on the left, the first issue label in the center, and the due date on the right.
